	...
	public void setDirectionInUserInterfaceDimensionUnit(float degrees) {
		...
		switch (rotationMode) {
			case ROTATION_STYLE_LEFT_RIGHT_ONLY:
				setRotation(convertCatroidAngleToStageAngle(0f));
				boolean change_to_right = oldRotation < 0 && newRotation >= 0;
				boolean change_to_left = oldRotation >= 0 && newRotation < 0;
				if (change_to_left || change_to_right) {
					lookData.getTextureRegion().flip(true, false);
					isFlipped = !isFlipped;
				}
				break;
			case ROTATION_STYLE_ALL_AROUND:
				setRotation(convertCatroidAngleToStageAngle(rotation));
				break;
